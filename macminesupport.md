...
1. Open `MacMine.xcodeproj` in Xcode 26.6.
2. Select the `MacMine` scheme.
3. Select `My Mac` as the run destination.
4. Press Run.

MacMine is a menu-bar app, so it appears in the macOS menu bar. Use **Open Dashboard** from that menu to show the main window.
...
swift build
swift test
xcodebuild -scheme MacMine -destination 'platform=macOS,arch=arm64' build
```

## App Store Materials

For App Store review notes, external API lists, third-party license notes, and an in-depth feature list, see:

```text
APP_STORE_README.md
```

For Xcode Cloud setup, including how to force stable Xcode 26.6 instead of Xcode 27 beta, see:
